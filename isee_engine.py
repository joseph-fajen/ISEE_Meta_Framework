...
This module contains the core ISEE (Idea Synthesis and Extraction Engine) logic
that can be imported directly by both CLI and web applications.

Extracted from main.py as part of Phase 3 refactoring to enable:
- Direct imports from app.py (no subprocess)
...
class ExecutionParams:
    ...
    This dataclass provides a clean interface for configuring ISEE runs
    without the complexity of argparse.Namespace objects.
    """
    query: str
    config_path: str = "globant_enterprise_config.json"
...
class ISEEApplication:
    ...
    This class orchestrates the entire ISEE pipeline:
    1. Generate combinations of models, frameworks, and domains
    2. Execute API calls (parallel or sequential)
    3. Evaluate results with scoring framework
    4. Synthesize top ideas
    ...
    def run_from_params(self, params: ExecutionParams) -> str:
        """Run the complete ISEE pipeline from an ExecutionParams object.

        This method provides a cleaner interface for running ISEE from web applications
        without needing to pass many individual arguments.

        Args:
            params: ExecutionParams dataclass with all execution parameters.

        Returns:
            Formatted output of synthesized ideas.
        """
        # Set provider mode
        self.set_provider_mode(params.provider)

        # Set skip rank files flag
        self.skip_rank_files = params.skip_rank_files

        # Set specific template IDs if provided
        if params.specific_template_ids:
        ...
        return self.run_complete_pipeline(
            ...
            query_variations=params.query_variations,
            max_combinations=params.max_combinations,
            output_format=params.output_format,
            use_real_models=params.use_real_models,
            specific_template_ids=params.specific_template_ids,
            verbose_queries=params.verbose_queries,
            show_all_queries=params.show_all_queries,
